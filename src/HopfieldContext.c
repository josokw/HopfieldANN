...
bool hopfield_save_weights(const HopfieldContext *ctx, const char *filename)
{
    ...
    for (int i = 0; i < ctx->patternSize; i++) {
       if (fwrite(ctx->W[i], sizeof(double), ctx->patternSize, f) != (size_t)ctx->patternSize) {
          fclose(f);
          return false;
       }
    }
...
bool hopfield_load_weights(HopfieldContext *ctx, const char *filename)
{
    ...
    if (magic[0] != 'H' || magic[1] != 'O' || magic[2] != 'P' || magic[3] != 'F') {
       fclose(f);
       return false;
    }
    ...
    /* If context already has matching dimensions, preserve patterns and just load W */
    bool dimensionsMatch = (ctx->nRows == nRows && ctx->nColumns == nColumns &&
    ...
    for (int i = 0; i < patternSize; i++) {
       if (fread(ctx->W[i], sizeof(double), patternSize, f) != (size_t)patternSize) {
          fclose(f);
          return false;
       }
    }
