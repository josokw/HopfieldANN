...
static int run_single_pattern(HopfieldContext *ctx, int patternIndex,
                                ...
                                SimState *state, bool *converged, bool firstPattern, bool mode2);
...
int run_cli(HopfieldContext *ctx, int argc, char *argv[])
{
    ...
    if (cfg.batchMode) {
       ...
       if (cfg.saveWeightsFile) {
          if (!cfg.quiet) {
             printf("- Saving weight matrix to %s .... ", cfg.saveWeightsFile);
          }
       ...
       if (argc - optind == 2) {
          /* Mode 2: noisy file provided, interactive noisy pattern selection */
          int menu = 0;
          bool firstRun = true;
          ...
          while (menu != 'E' && menu != 'e') {
             ...
             if (!cfg.quiet) {
                printf("- E(xit), N(ext simulation), R(un again), "
                       "<Enter>(repeat) .... ");
             }
             if (!enterPressed) {
                clearInput();
                             ...
                             LearningRule rule, SimState *state)
{
   ...
   printf("ready\n\n");
   print_neurons(ctx);
   return prepare_network(ctx, rule, state, false);  /* interactive mode: not quiet */
}
...
/* Parse a comma-separated list of integers. Returns allocated array and count. */
...
static bool parse_cli_options(int argc, char *argv[], BatchConfig *cfg)
{
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "r:p:n:s:qvho:w:l:", long_options, NULL)) != -1) {
       switch (opt) {
       case 'r': {
          if (strcasecmp(optarg, "hebbian") == 0) cfg->rule = RULE_HEBBIAN;
          else if (strcasecmp(optarg, "storkey") == 0) cfg->rule = RULE_STORKEY;
          else if (strcasecmp(optarg, "pseudo-inverse") == 0) cfg->rule = RULE_PSEUDO_INVERSE;
          else if (strcasecmp(optarg, "daydreaming") == 0) cfg->rule = RULE_DAYDREAMING;
          else if (strcasecmp(optarg, "modern") == 0) cfg->rule = RULE_MODERN;
          else {
       ...
       case 'n': {
          ...
          if (endptr == optarg || *endptr != '\0' || val < 0 || val > MAX_NOISE_PERCENT) {
       ...
       case 'h': cfg->helpRequested = true; break;
       case 'o': cfg->outputFile = strdup(optarg); break;
       case 'w': cfg->saveWeightsFile = strdup(optarg); break;
       case 'l': cfg->loadWeightsFile = strdup(optarg); break;
       case '?': return false;
       default: return false;
       }
    }

    /* Batch mode if pattern(s) specified (mode 1) or pattern specified (mode 2) */
    cfg->batchMode = (cfg->nPatterns > 0);

    /* --load-weights and --rule are compatible but warn if both set */
    if (cfg->loadWeightsFile && cfg->ruleSet) {
       if (!cfg->quiet) {
          fprintf(stderr, "- Note: --load-weights used with --rule; "
       ...
       while (fgets(line, sizeof(line), f)) {
          ...
          *eq = '\0';
          ...
          if (strcasecmp(key, "rule") == 0) {
             if (strcasecmp(val, "hebbian") == 0) settings->rule = RULE_HEBBIAN;
             else if (strcasecmp(val, "storkey") == 0) settings->rule = RULE_STORKEY;
             else if (strcasecmp(val, "pseudo-inverse") == 0) settings->rule = RULE_PSEUDO_INVERSE;
             else if (strcasecmp(val, "daydreaming") == 0) settings->rule = RULE_DAYDREAMING;
             else if (strcasecmp(val, "modern") == 0) settings->rule = RULE_MODERN;
             ...
             if (endptr != val && *endptr == '\0' && v >= 0 && v <= MAX_NOISE_PERCENT) {
                settings->noise = (int)v;
                settings->noiseSet = true;
             }
          } else if (strcasecmp(key, "verbose") == 0) {
             settings->verbose = (strcasecmp(val, "true") == 0 || strcasecmp(val, "1") == 0 || strcasecmp(val, "yes") == 0);
...
static bool apply_config_to_batch(const ConfigFileSettings *config, BatchConfig *cfg)
{
...
static int run_batch_mode(HopfieldContext *ctx, const BatchConfig *cfg, SimState *state, bool mode2)
{
    ...
    if (cfg->loadWeightsFile) {
       if (!cfg->quiet) {
          printf("- Loading weight matrix from %s .... ", cfg->loadWeightsFile);
       }
    ...
    for (int i = 0; i < cfg->nPatterns; i++) {
       ...
       int ret = run_single_pattern(ctx, cfg->patterns[i], cfg->noise, cfg, state, &converged, firstPattern, mode2);
       if (ret < 0) return ret;  /* -1 = error, -3 = invalid index */
                               ...
                               SimState *state, bool *converged, bool firstPattern, bool mode2)
{
    if (mode2) {
       ...
       copyPattern(ctx->patternSize, ctx->noisyPatterns[patternIndex], state->inputPattern);
       copyPattern(ctx->patternSize, ctx->noisyPatterns[patternIndex], state->inputPatternWithNoise);
       ...
       copyPattern(ctx->patternSize, ctx->patterns[patternIndex], state->inputPattern);
       copyPattern(ctx->patternSize, ctx->patterns[patternIndex], state->inputPatternWithNoise);
    ...
    ConvergenceCallback callback = cfg->verbose ? verbose_iteration_callback : NULL;
    *converged = convergePattern(ctx, state->inputPatternWithNoise, state->outputPattern,
                                 callback, &cb_data, &finalEnergy);

    if (!cfg->quiet) {
       double overlap = calcOverlap(ctx->patternSize, state->inputPattern, state->outputPattern);
       int hamming = calcHammingDistance(ctx->patternSize, state->inputPattern, state->outputPattern);
       print_batch_result(patternIndex + 1, overlap, hamming, ctx->patternSize, *converged);
    }

    if (cfg->outputFile) {
       if (!save_recall_output(ctx, state->outputPattern, cfg->outputFile, firstPattern)) {
                               ...
                               bool firstPattern)
{
    ...
    for (int r = 0; r < ctx->nRows; r++) {
       for (int c = 0; c < ctx->nColumns; c++) {
          fprintf(f, "%c", equals(pattern[r * ctx->nColumns + c], 1.0) ? '*' : '.');
       }
       fprintf(f, "\n");
    }
                               ...
                               int patternSize, bool converged)
{
    printf("  Pattern %d: overlap=%7.4f hamming=%d/%d converged=%s\n",
           patternIdx, overlap, hamming, patternSize, converged ? "yes" : "no");
}
...
static void print_usage(void)
{
    ...
    printf("Options:\n");
    printf("  -r, --rule RULE           Learning rule: hebbian, storkey, pseudo-inverse,\n");
    printf("                            daydreaming, modern (default: prompt)\n");
    printf("  -p, --pattern LIST        Pattern indices 1..N, comma-separated (e.g. 1,3,5)\n");
    printf("  -n, --noise PERCENT       Noise level 0..100 (mode 1 only)\n");
    ...
    printf("  -w, --save-weights FILE   Save weight matrix after learning (.bin)\n");
    printf("  -l, --load-weights FILE   Load weight matrix, skip learning\n");
    printf("  -h, --help                Show this help\n\n");
    printf("Config file: ./.hopfieldrc or ~/.hopfieldrc (key=value, # comments)\n");
    printf("  rule, seed, noise, verbose, save_weights, load_weights, output\n\n");
